...
;
; An update by TobyLobster to the original Starship Command (1983) by Peter Irvin.
;
...
cosine_fraction                             = $0d   ; }
cosine_pixels                               = $0e   ; } for rotating powerups (reusing locations above)
cosine_screens                              = $0f   ; }
...
resultx_fraction                            = $17   ; }
resultx_pixels                              = $18   ; }
resultx_screens                             = $19   ; } for rotation calculations
resulty_fraction                            = $1a   ; }
resulty_pixels                              = $1b   ; }
resulty_screens                             = $1c   ; }

x_pixels                                    = $17   ; reusing memory locations above
y_pixels                                    = $18   ;           "
...
sine_fraction                               = $1f   ; }
sine_pixels                                 = $20   ; } for rotating powerups
sine_screens                                = $21   ; }
...
result                                      = $30
end_low                                     = $31   ; used in decompressing text
end_high                                    = $32
starship_low                                = $33   ; }
...
starship_rotation_powerup_cosine            = $fe
...
;  starship_rotation_cosine_table[i] = <ROUND(65536*COS(i * 0.44444444444 degrees))
;    starship_rotation_sine_table[i] = <ROUND(65536*SIN(i * 0.44444444444 degrees)))
;
; At maximum angle of rotation (2.22222222 degrees), this means the ship will complete a full 360 degrees in 162 steps of rotation.
; ----------------------------------------------------------------------------------
starship_rotation_powerup_cosine_table
    !byte 0  ,   2,  13,  23,  37,  54                                ; roughly 65536 - 65536*COS(i * 0.44444444444 degrees), but tweaked slightly
                                                                      ; in 'Processing' application to make it look slightly better.
...
+
    ...
    lda starship_rotation_powerup_cosine_table,y                      ;
    sta starship_rotation_powerup_cosine                              ;
...
unplot_long_range_scanner_if_shields_inactive
    ...
    jsr plot_enemy_ships_and_powerups_on_scanners                     ;
...
sound_with_volume
    ...
    clc                                                               ;
    adc #1                                                            ;
    sta sounds_high+2, x                                              ; volume low byte
    beq set_volume_high                                               ;
    lda #$ff                                                          ;
set_volume_high
    sta sounds_high+3, x                                              ; volume high byte
do_osword_sound
...
; ----------------------------------------------------------------------------------
; Show powerup on the long range scanner and short range scanner
;
; On Entry:
;   A = X screens
;   Y = Y screens
...
;   i.e. which screen it's in, i.e. which of the 25 squares in the short range scanner to use
; ----------------------------------------------------------------------------------
plot_or_unplot_powerup_on_scanner
    stx set_pixel_flag                                                ;
...
; ----------------------------------------------------------------------------------
plot_enemy_ships_and_powerups_on_scanners
    lda #maximum_number_of_enemy_ships                                ;
    sta enemy_ships_still_to_consider                                 ;
    ldx #0                                                            ;
plot_enemy_ships_and_powerups_on_scanners_loop
...
skip_plotting_enemy_ship_on_scanner
    ...
    inx                                                               ;
    dec enemy_ships_still_to_consider                                 ;
    bne plot_enemy_ships_and_powerups_on_scanners_loop                ;

    jsr plot_powerups_on_scanners                                     ;
...
; ----------------------------------------------------------------------------------
plot_powerups_on_scanners
    lda #maximum_number_of_powerups                                   ;
    sta powerups_still_to_consider                                    ;
    ldx #0                                                            ;
plot_powerups_on_scanners_loop
    stx temp8                                                         ; powerup index
    lda powerups_previous_x_screens,x                                 ; x screen
    ldy powerups_previous_y_screens,x                                 ; y screen
    ldx #0                                                            ; unplot
    jsr plot_or_unplot_powerup_on_scanner                             ;
skip_unplotting_powerups_on_scanner
    lda starship_shields_active                                       ;
    beq skip_plotting_powerup_on_scanner                              ;
    ldx temp8                                                         ; powerup index
    lda powerups_taken,x                                              ;
    bne skip_plotting_powerup_on_scanner                              ; skip if taken
    ...
    ldy powerups_y_screens,x                                          ; y screen
    ldx #$ff                                                          ; plot
    jsr plot_or_unplot_powerup_on_scanner                             ;
skip_plotting_powerup_on_scanner
    ldx temp8                                                         ; powerup index
    inx                                                               ;
    dec powerups_still_to_consider                                    ;
    bne plot_powerups_on_scanners_loop                                ;
    rts                                                               ;
...
skip_addition
    ...
    clc                                                               ;
    adc #$10                                                          ; not range 0 to (max-1) but range 1 to max
...
!if do_debug {
    ...
    jsr plot_enemy_ships_and_powerups_on_scanners                     ;
skip_scanner_update
    jsr update_powerups_previous_position                             ;
!if elk=0 {
wait_for_timer
...
update_stars_loop
    ...
    ldx x_pixels                                                      ;
maybe_unplot_star
    jsr eor_play_area_pixel                                           ; unplot. Target of self modifying code. Skipped on first execution.
    ldy current_object_index                                          ;
...
update_object_position_for_starship_rotation_and_speed
    ...
    beq add_starship_velocity_to_object_position                      ; no rotation
...
!if (maximum_number_of_powerups * 6 > 129) {
...
!if debug_powerups = 0 {
    ...
    lda #0                                                            ;
    ldx #maximum_number_of_powerups * 6 - 1                           ;
-
...
; ----------------------------------------------------------------------------------
update_powerups_previous_position
...
starship_is_rotating1
    ...
    sbc #$7f                                                          ;
    sta powerups_x_pixels,x                                           ;
    lda powerups_previous_x_screens,x                                 ;
    sbc #$7f                                                          ;
    sta powerups_x_screens,x                                          ;

    lda powerups_previous_y_fraction,x                                ;
    sta powerups_y_fraction,x                                         ;
    lda powerups_previous_y_pixels,x                                  ;
    sec                                                               ;
    sbc #$7f                                                          ;
    sta powerups_y_pixels,x                                           ;
    lda powerups_previous_y_screens,x                                 ;
    sbc #$7f                                                          ;
    sta powerups_y_screens,x                                          ;
...
+
    ...
    ; rotate
    jsr multiply_powerup_position_by_starship_rotation_sine           ; (sine_fraction, sine_pixels, sine_screens) = posY * sin(angle)

    ldx temp_y                                                        ; point back to X coordinates again
    jsr multiply_powerup_position_by_starship_rotation_cosine         ; (cosine_fraction, cosine_pixels, cosine_screens) = posX * cos(angle)
    ...
    jsr multiply_powerup_position_by_starship_rotation_sine           ; (sine_fraction, sine_pixels, sine_screens) = posX * sin(angle)

    ; X = look at Y coordinates again
    ldx result                                                        ; recall offset to Y coordinates
    jsr multiply_powerup_position_by_starship_rotation_cosine         ; (cosine_fraction, cosine_pixels, cosine_screens) = posY * cos(angle)
...
command_move_string
    ...
    !byte $19, 4                                                      ; MOVE 1123 or 1139, 129
command_move_string_horizontal_pos
    !word 1139                                                        ; This centres the command number
    !word 129                                                         ; underneath the word 'Command'
